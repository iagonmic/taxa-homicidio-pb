...
```{python echo=FALSE}
import pandas as pd
sexo = pd.DataFrame({'Taxa': ['92,56%', '7,44%']}, index=['Masculino', 'Feminino'])
display(sexo)
```
...
```{python echo=FALSE}
import pandas as pd
ocup = pd.DataFrame({'Ocupação' : ['Desempregado ou trabalhador informal', 'Trabalhador rural', 'Servente de Obras', 'Caseiro', 'Pedreiro']}, index=['1º', '2º', '3º', '4º', '5º'])
display(ocup)
```
...
- 21.20% das vítimas possuiam apenas o Ensino Fundamental 1.

- Dessas, 34,69% possuiam entre 25 e 35 anos de idade.
...
Sintetizando, a maior parte das vítimas possui até o Ensino Fundamental 1, na faixa etária de 25 a 35 anos, são homens pardos, que trabalham na área rural
...
Após diversas análises, percebeu-se que as principais vítimas de homicídio são aquelas que possuem algum tipo de vulnerabilidade social, que reflete vulnerabilidades econômicas. 

Assim, evidencia-se a relação entre os fatores sociais analisados, com as taxas de homicídio no estado da Paraíba.

## Limitações

- Dados faltantes ou ignorados

- Sem dados de escolaridade dos anos de 2010 e 2011

## Sugestões

- Junção dos dados do SIM com os dados de mortalidade da Secretaria Estadual de Segurança Pública e Defesa Social.
